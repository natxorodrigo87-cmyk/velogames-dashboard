...
import { Send, Globe, BookOpen, Database, Loader2, RefreshCw, Key, AlertTriangle } from 'lucide-react';
...
const CyclingAI: React.FC<CyclingAIProps> = ({ mode, onClose }) => {
  ...
  useEffect(() => {
    const welcome = mode === 'pcs' 
      ? '¡Radio Tour 2026 listo! Pregúntame sobre cualquier carrera actual en procyclingstats.com.'
      : 'Has entrado en la Biblioteca Frikis. ¿Qué hito histórico del ciclismo quieres que analice?';
  ...
  const handleSend = async (manualText?: string) => {
    ...
    setLoading(true);
    setStatus('Sintonizando frecuencia...');

    try {
      // SIEMPRE inicializar dentro de la función para pillar la API_KEY más reciente del puente Netlify
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY || '' });
      
      const systemInstruction = mode === 'pcs'
        ? 'Eres el analista técnico de la Liga Frikis. Usa Google Search para extraer datos de procyclingstats.com. Sé muy preciso con los resultados de 2026.'
        : 'Eres el historiador oficial de la Liga Frikis. Responde con épica sobre ciclismo clásico.';

      // gemini-3-pro-preview es el modelo más estable para googleSearch
      const modelName = mode === 'pcs' ? 'gemini-3-pro-preview' : 'gemini-3-flash-preview';
      ...
      const botText = response.text || "Se ha cortado la comunicación con el coche de equipo.";
      ...
      if (chunks) {
        chunks.forEach((c: any) => {
          if (c.web?.uri) {
            sources.push({ uri: c.web.uri, title: c.web.title || 'PCS Data' });
          }
        });
      }
      ...
      if (errorStr.includes("not found") || errorStr.includes("404") || errorStr.includes("api key") || errorStr.includes("401")) {
        setMessages(prev => [...prev, { 
          role: 'bot', 
          text: "¡CONEXIÓN FALLIDA! No detecto una llave activa de Google Cloud. Por favor, pulsa el botón de abajo para activar tu acceso.", 
          isError: true,
          needsKey: true
        }]);
      } else {
        setMessages(prev => [...prev, { 
          role: 'bot', 
          text: "¡PÁJARA DEL SERVIDOR! Hubo un error de red o cuota. Prueba a reenviar en unos segundos.", 
          isError: true 
        }]);
      }
    } finally {
  ...
  const handleOpenKey = async () => {
    ...
    if (window.aistudio) {
      ...
      // Pequeño retardo visual para indicar que se está reconectando
      setStatus('Reconectando...');
      setTimeout(() => {
        const lastMsg = [...messages].reverse().find(m => m.role === 'user');
        if (lastMsg) handleSend(lastMsg.text);
      }, 500);
    }
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-sm font-black uppercase italic tracking-tighter leading-none">
              {isPcs ? 'Radio Tour 2026' : 'Archivo Liga Frikis'}
            </h2>
              ...
              <div className={`p-4 rounded-2xl text-sm leading-relaxed shadow-xl ${
                ...
                {m.needsKey && (
                  <button 
                    ...
                    className="mt-4 flex items-center gap-3 w-full justify-center py-4 bg-white text-black rounded-xl font-black text-[10px] uppercase shadow-lg hover:bg-slate-100 transition-all active:scale-95"
                  >
                    <Key className="w-4 h-4" /> Conectar con Google Cloud
                  </button>
                )}
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {m.sources.map((s, idx) => (
                    <a key={idx} href={s.uri} target="_blank" rel="noopener noreferrer"
                      className="text-[9px] text-blue-400 font-black flex items-center gap-1.5 bg-blue-500/10 px-3 py-2 rounded-lg border border-blue-500/20 uppercase hover:bg-blue-500/20 transition-colors">
      ...
      <div className="shrink-0 p-4 bg-slate-950 border-t border-white/10 pb-12 md:pb-6">
        <div className="flex gap-2">
          <input 
            ...
            placeholder={isPcs ? "¿Qué carrera hay hoy?" : "¿Quién ganó en 1998?"}
        ...
        <p className="mt-2 text-[8px] text-center text-slate-600 uppercase font-black tracking-widest">Powered by Google Gemini 3 Pro</p>
      </div>
    </div>
  );
};
